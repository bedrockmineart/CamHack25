...
export default function PhonePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [showSingleDeviceWarning, setShowSingleDeviceWarning] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const onSingleDevicePlacement = (payload: any) => {
      setIsSingleDeviceMode(true);
      setShowSingleDeviceWarning(true);
      setMessage(payload.message || 'Single device mode - place phone on keyboard');
    };
    
    const onSingleDeviceOperation = () => {
      setIsSingleDeviceMode(true);
      setMessage('Single device operation - listening for keystrokes');
    };
    
    const onInferenceResult = (payload: any) => {
      console.log('[INFERENCE] Detected keys:', payload.predictions);
      // You can display these results in the UI if needed
    };
    ...
    socket.on('single-device-placement', onSingleDevicePlacement);
    socket.on('single-device-operation', onSingleDeviceOperation);
    ...
    return () => {
      ...
      socket.off('single-device-placement', onSingleDevicePlacement);
      socket.off('single-device-operation', onSingleDeviceOperation);
  ...
  return (
    <div style={{ padding: 16, fontFamily: 'system-ui' }}>
      <SingleDeviceWarning 
        show={showSingleDeviceWarning} 
        onConfirm={() => {
          setShowSingleDeviceWarning(false);
          // Call backend to start single-device operation
          const backend = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000';
          fetch(backend + '/api/session/start-single-device', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
          }).catch(err => console.error('Error starting single-device operation:', err));
        }}
      />
