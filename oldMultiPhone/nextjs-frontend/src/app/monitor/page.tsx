...
export default function MonitorPage() {
  ...
  const [isSingleDeviceMode, setIsSingleDeviceMode] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const onStatusUpdate = (status: any) => {
      ...
      setIsSingleDeviceMode(status.deviceMode === 'single');
    };
    ...
    // Listen for inference results in single-device mode
    const onInferenceResult = (data: any) => {
      console.log('[Monitor] Inference result:', data);
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {/* Single Device Mode Warning */}
      {isSingleDeviceMode && phase !== 'idle' && (
        <div style={{ marginTop: 20, padding: 16, background: '#fff3cd', borderRadius: 8, border: '2px solid #ffc107' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
            <span style={{ fontSize: 24 }}>⚠️</span>
            <h4 style={{ margin: 0, color: '#856404' }}>Single Device Mode Active</h4>
          </div>
          <p style={{ margin: '0 0 8px 0', fontSize: 14, color: '#856404' }}>
            Only one device is connected. The system is using <strong>machine learning inference</strong> for keystroke detection.
          </p>
          <p style={{ margin: 0, fontSize: 13, color: '#856404', fontStyle: 'italic' }}>
            ℹ️ Phone must be placed in the center of the keyboard with microphone facing down. No calibration needed.
          </p>
          
          {/* Display recent inference results */}
          {inferenceResults.length > 0 && (
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid #ffc107' }}>
              <h5 style={{ margin: '0 0 8px 0', color: '#856404' }}>🎯 Detected Keystrokes:</h5>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {inferenceResults.slice(0, 5).map((result, idx) => (
                  <div
                    key={idx}
                    style={{
                      padding: '8px 12px',
                  ...
                  >
                    ...
                    <span style={{ fontSize: 16, fontWeight: 600, color: '#333', fontFamily: 'monospace' }}>
                      {result.keys.join(', ')}
                    </span>
                  </div>
                ))}
