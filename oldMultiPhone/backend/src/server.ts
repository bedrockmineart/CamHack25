...
// Single-device inference endpoints
app.post('/api/session/start-single-device', (_req, res) => {
    const result = phaseService.startSingleDeviceOperation();
    // Also start the inference service
    const inferenceService = require('./services/inferenceService');
    const devices = socketServer.getConnectedDevices();
    if (devices.length === 1) {
        inferenceService.startInference(devices[0]);
    }
    res.json(result);
});
