...
async function runPythonInference(audioFile: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    const scriptPath = path.join(__dirname, '../../algorithm/run_inference.py');
    ...
    pythonProcess.on('close', (code) => {
      if (code !== 0) {
        ...
        if (stderr) console.error(`[INFERENCE] stderr: ${stderr}`);
        resolve([]);
        return;
      }
      ...
      try {
        ...
        if (Array.isArray(predictions)) {
          ...
          console.error('[INFERENCE] Invalid output format (expected array):', output);
          resolve([]);
        }
      } catch (error) {
